...
Loops work great when we want to fill arrays with data. We can make some pretty interesting stuff happen like web scraping data we need and adding it to a list.
    • The code must be in external javascript student_id.js
    • The javaScript code must be uploaded to gblearn.com "comp1231/lab/tests/test1" folder and my.gblearn.com (for similarity test) 
        ◦ Students must selected an assign version when uploading their code to my.gblearn.com 
            ▪ Jordan Version E, F, G, H
